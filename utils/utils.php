      ...
      foreach ($ref_docs as $key => $ref_doc):
        ...
        if (count($ref_doc_metadata) > 0):
            foreach ($ref_doc_metadata as $key => $metadata): ?>
                    <li><a target="_blank" href="<?php echo wpckan_get_ckan_domain().'/dataset/'.$metadata['name'] ?>"><?php echo getMultilingualValueOrFallback($metadata['title_translated'], get_current_language()) ?></a>
    ...
    <table id="reference" class="data-table">
       ...
       foreach ($ref_docs as $key => $ref_doc):
        ...
        if (count($ref_doc_metadata) > 0):
           foreach ($ref_doc_metadata as $key => $metadata): ?>
               <tr>
                 <td class="row-key">
                   <a target="_blank" href="<?php echo wpckan_get_ckan_domain().'/dataset/'.$metadata['name'] ?>"><?php echo getMultilingualValueOrFallback($metadata['title_translated'], get_current_language()) ?></a></br>
                   <div class="ref_date">
                     <?php if ($metadata['type'] == 'laws_record' && !(IsNullOrEmptyString($metadata['odm_promulgation_date']))): ?>
                       <?php   if (get_current_language() == 'km') {
      echo convert_date_to_kh_date(date('d/m/Y', strtotime($metadata['odm_promulgation_date'])), '/');
      ...
      echo '('.$metadata['odm_publication_date'].')';
        ...
        ?>
                 ...
                 <td><?php echo getMultilingualValueOrFallback($metadata['notes_translated'], get_current_language());
        ?></td>
               </tr>
           <?php
           endforeach;
